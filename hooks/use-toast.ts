...
function dispatch(action: any) {
  ...
  if (type === "DISMISS_TOAST") memoryState = { toasts: memoryState.toasts.map(t => t.id === toastId || !toastId ? { ...t, open: false } : t) }
...
export function toast(props: Omit<ToasterToast, "id">) {
  ...
  dispatch({ type: "ADD_TOAST", toast: { ...props, id, open: true, onOpenChange: (open: boolean) => { if (!open) { setTimeout(() => dispatch({ type: "REMOVE_TOAST", toastId: id }), TOAST_REMOVE_DELAY) } } } })
  return { id, dismiss }
}
